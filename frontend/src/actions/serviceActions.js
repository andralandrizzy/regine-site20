...
import {
    ...
    SERVICE_UPDATE_FAIL,
    SERVICE_HOME_REQUEST,
    SERVICE_HOME_SUCCESS,
    SERVICE_HOME_FAIL,
...
export const listHomeService = () => async (dispatch) => {
    try {
        dispatch({ type: SERVICE_HOME_REQUEST })

        const { data } = await axios.get(`/api/services/home`)

        dispatch({
            type: SERVICE_HOME_SUCCESS,
            payload: data,
        })
    } catch (error) {
        dispatch({
            type: SERVICE_HOME_FAIL,
            payload:
                error.response && error.response.data.message
                    ? error.response.data.message
                    : error.message,
